document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Pomoćna funkcija za renderiranje HTML-a rezultata (OVA FUNKCIJA SE SADA NE KORISTI NAKON INICIJALNOG POKRETANJA) ---
    // Drži je ovdje samo radi strukturne jasnoće ako se ikad budeš vratio na nju
    // Ali ona ne smije biti pozvana unutar izracunajMarkere()
    function renderResultsHtml() {
        // Ova funkcija je bila problematična jer je prebrisala DOM.
        // Njezina uloga je sada preuzeta direktnim manipuliranjem textContenta.
        // Ostavljamo je praznu ili se može u potpunosti izbrisati ako se više ne koristi.
    }

    // Inicijalno postavi defaultne vrijednosti (ove vrijednosti su sada već u HTML-u)
    // Ovdje se ne poziva renderResultsHtml() jer su elementi već dohvaćeni.
    ...
    function izracunajMarkere() {
        ...
        if (brojBeatova === 0) {
            // I ovo je specifična poruka greške, pa je obradi slično
    ...
    izracunajMarkere();
});.addEventListener('change', izracunajMarkere);
    mjeraTaktaSelect.addEventListener('change', izracunajMarkere);

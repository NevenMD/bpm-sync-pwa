document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateAndDownloadXML() {
        ...
        xmlContent += `    <edius:formatVersion>4</edius:formatVersion>${CRLF}`; // Changed from \t to 4 spaces
        ...
        xmlContent += `    <edius:CreateDate>${dayOfWeek} ${month} ${dayOfMonth} ${hours}:${minutes}:${seconds} ${year}</edius:CreateDate>${CRLF}`; // Changed from \t to 4 spaces
        
        xmlContent += `    <edius:markerLists>${CRLF}`; // Changed from \t to 4 spaces
        ...
        edlMarkers.forEach((marker, index) => {
            ...
            xmlContent += `        <edius:marker>${CRLF}`; // Changed from \t\t to 8 spaces
            xmlContent += `            <edius:no>${markerNo}</edius:no>${CRLF}`; // Changed from \t\t\t to 12 spaces
            xmlContent += `            <edius:anchor>${anchorValue}</edius:anchor>${CRLF}`; // Changed from \t\t\t to 12 spaces
            xmlContent += `            <edius:position>${positionTimecode}</edius:position>${CRLF}`; // Changed from \t\t\t to 12 spaces
            xmlContent += `            <edius:duration>${durationValue}</edius:duration>${CRLF}`; // Changed from \t\t\t to 12 spaces
            xmlContent += `            ${commentXml}${CRLF}`; // Changed from \t\t\t to 12 spaces
            xmlContent += `            <edius:color>${colorValue}</edius:color>${CRLF}`; // Changed from \t\t\t to 12 spaces
            xmlContent += `        </edius:marker>${CRLF}`; // Changed from \t\t to 8 spaces
        });
